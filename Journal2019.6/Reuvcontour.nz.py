...
for time in timeList:
    ...
    vtemp = tM.trs(wake[case].wakeData[time][sec][:,1:])[:,4]
    for i in range(RS.shape[0]):
        RS[i,0] += abs((utemp[i] - wake_ave[case][:,4][i]) * (vtemp[i] - wake_ave[case][:,5][i]))
RS /= len(timeList) * np.power(11.4,2)
RS = np.hstack((wake_ave[case][:,1:4], RS))

''' save RS into a file with pickle '''
import pickle
f = open(projDir + 'postProcessing_all/data.processed/' + caseName[case] + '_' + sec + '_RSuv', 'wb')
pickle.dump(RS, f)
f.close()
# # 读取 RS
# f = open(projDir + 'postProcessing_all/data.processed/' + caseName[case] + '_' + sec + '_RSuv', 'rb')
# RS = pickle.load(f) # all wake information of the case
# f.close()

# 必须让RS也符合n*7的矩阵，才能网格化
RS = np.hstack((wake_ave[case][:,1], RS))
...
RS = SecITP(RS)
RS.meshITP_Nz((-378, 1512, 1890), (-252, 252, 504))
''' u_ave for PlaneZ'''
x = {}
y = {}
z = {}
